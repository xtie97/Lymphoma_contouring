...
'''
Threshold: Union of 40% SUV max and 2.5 SUV 
'''
def generate_contours(dicom_series_path, rtstruct_path, nifti_img_path, save_rtstruct_path, dil):
    ...
    for roi_name in roi_name_list:
        ...
        roi_new_name = 'ut1_dil{}_'.format(dil) + roi_name
        if dil == 1:
            color_code = [138,43,226] 
        elif dil == 2: 
            color_code = [255,48,48]
        elif dil == 3: 
            color_code = [238,180,34]
    ...
    ds.SeriesDescription = 'Union_threshold_2.5_SUV_0.4_SUVmax_dilate_{}voxel'.format(dil)
    with open(save_rtstruct_path, 'wb') as outfile:
        ds.save_as(outfile)

def process_contouring(root_dir, nifti_path, dil):
    save_rtstruct_folder = '/UserData/Lymphoma_UW_Retrospective/Data/uw_analyzed/Xin/rtstruct/union_threshold_01_dilate_{}voxel/'.format(dil)
...
if "__main__" == __name__:     
    ...
    process_contouring(root_dir, nifti_path, dil=1)
    process_contouring(root_dir, nifti_path, dil=2)
    process_contouring(root_dir, nifti_path, dil=3)
